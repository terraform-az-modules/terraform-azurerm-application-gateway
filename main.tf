...
##-----------------------------------------------------------------------------
## Application Gateway Resource With Lifecycle Rules
##-----------------------------------------------------------------------------
resource "azurerm_application_gateway" "main_with_lifecycle" {
  count               = var.enabled && var.enable_ignore_changes ? 1 : 0
...
##-----------------------------------------------------------------------------
## Application Gateway Resource Without Lifecycle Rules
##-----------------------------------------------------------------------------
resource "azurerm_application_gateway" "main_without_lifecycle" {
  count               = var.enabled && !var.enable_ignore_changes ? 1 : 0
  name                = var.resource_position_prefix ? format("appgw-%s", local.name) : format("%s-appgw", local.name)
  resource_group_name = var.resource_group_name
  location            = local.location
...
resource "azurerm_private_endpoint" "pep" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = local.application_gateway_id
...
resource "azurerm_monitor_diagnostic_setting" "appgw-law" {
  ...
  target_resource_id         = local.application_gateway_id
